...
%   for its use.
%
% File name ShearWave_WT.m:
% Generate .mat Sequence Object file for L7-3 Linear flash transmit with Shearwave visualization.
...
% 注意：1208行定义了一个叫SWIImage的函数，主要用于计算杨氏模量并实时显示
clear all
...
% 定义十个聚焦位置(-20,80),(-20,90),(-20,100),(-20,110),(-20,120)
% (20,80),(20,90),(20,100),(20,110),(20,120)
SWIFocusX = [-20,20];  
SWIFocusZ = [80,90,100,110,120];  

% ROI的宽度和高度，单位波长
SWIROI.width = 100; 
SWIROI.height = 100;  % [width and height in wavelength]

Line_Num = length(SWIFocusX);
Point_Num = length(SWIFocusZ);

% 追踪帧数，一共追踪200帧
% 左侧聚焦完5次，追踪100帧，接着右边聚焦5次，追踪100帧，合计200帧
na          = 100 * Line_Num;      % Set na = number of detect acquisitions.
SWIFrames   = 2;
BmodeFrames = 2;
...
% Specify PData(2) structure array for Shearwave visulization %剪切波可视化窗口的设置
PData(2) = PData(1);
...
PData(2).Region.Shape = struct(...
    'Name','Rectangle',...
    'Position',[0,0,SWIFocusZ(ceil(Point_Num/2))-SWIROI.height/2],...
    'width', SWIROI.width,...
...
TX = repmat(struct('waveform', 2, ...
    ...
    'Delay', zeros(1,Trans.numelements)), 1, 2 + Line_Num * Point_Num);
% - Set event specific TX attributes for push.
TX(1).Apod=[zeros(1,32),ones(1,64),zeros(1,32)];
TX(1).waveform = 1;
TX(2).focus = SWIFocusZ(ceil(Point_Num/2));       % wavelength, can be changed in the GUI
TX(2).focusX = 0;              % can be changed in the GUI
TX(2).FocalPt = [TX(2).focusX,0,TX(2).focus];
TX(2).pushElements = 64;       % can be changed in the GUI
TX(2).Apod(64-TX(2).pushElements/2 +1 : 64+TX(2).pushElements/2) = 1;
TX(2).Delay = computeTXDelays(TX(2));
% 多线多点聚焦定义
for i=3:length(TX)
        TX(i).focus = SWIFocusZ(mod(i-3,Point_Num)+1);       % wavelength, can be changed in the GUI
        TX(i).focusX = SWIFocusX(floor((i-3)/Point_Num)+1);      % can be changed in the GUI
        TX(i).FocalPt = [TX(i).focusX,0,TX(i).focus];
        TX(i).pushElements = min(64,TX(i).focus);       % can be changed in the GUI
        left=max(1,  64+TX(i).focusX-TX(i).pushElements/2 +1);
        right=min(128,64+TX(i).focusX+TX(i).pushElements/2 +1);
        TX(i).Apod(33 : 96) = 1;
...
SeqControl(5).command = 'timeToNextEB';
SeqControl(5).argument = 150;               % 500usec
NextEB = 5;
...
SeqControl(7).argument = 500000;            % 1000000usec = 1000msec (~ 5 fps)
TTNEB=7;
...
SeqControl(9).condition = 'Trigger_1_Rising'; % Trigger input 1, enable with rising edge
SeqControl(9).argument = 0; % 500 msec timeout delay (Timeout range is 1:255 in 250 msec steps; 0 means timeout disabled)

SeqControl(10).command = 'timeToNextAcq';
SeqControl(10).argument = 500;               % 500usec
AfterPush = 10;

SeqControl(11).command = 'timeToNextEB';
SeqControl(11).argument = 300000;               % 500usec
BT2Line = 11;
...
for i = 1:SWIFrames  
    j=0;
    for nLine=1:Line_Num
        % 聚焦
        for nPush = 1:Point_Num
            Event(n).info = 'Push transmit';
            Event(n).tx = 2 + (nLine-1) * Point_Num + nPush;
            Event(n).rcv = 0;
            Event(n).recon = 0;
            Event(n).process = 0;
            Event(n).seqControl = NextEB;
            n = n+1;
        end
        Event(n-1).seqControl = [AfterPush,BT2Line];
        if nLine==Line_Num
            Event(n-1).seqControl = [AfterPush,TTNEB];
        end
        % 追踪
        for j = j+1:nLine*floor(na/Line_Num)          % Acquire frame
...
UI(15).Control = VsSliderControl('LocationCode','UserB3','Label','Push cycles',...
                  'SliderMinMaxVal',[1,2000,pushCycle],...
...
EF(3).Function = vsv.seq.function.ExFunctionDef('SWIImage', @SWIImage);
% ROI position
...
function processIQ(IBuffer, QBuffer)
    ...
    if ishandle(bmodeFigHandle)
        if  isempty(recHandle) || ~ishandle(recHandle)
            ...
            recHandle = rectangle('Position',[x-SWIROI.width/2+18,z-SWIROI.height/2+3.875,SWIROI.width-36,SWIROI.height-7.75],'EdgeColor','w','LineWidth',2);
...
%             markHandle = plot(x,z,'xw','MarkerFaceColor','r','MarkerSize',8,'Linewidth',2);hold off;
    ...
    if newIQ == 1  % Need to replot focus IQ data after changing ROI
        ...
        title('Shear Wave Visualization with L7-3','FontSize',14,'Fontweight','bold');
...
%实时成像，算法原理看cpu版本里的代码
function SWIImage(IBuffer,QBuffer)
    PData = evalin('base','PData');
    SWIROI = evalin('base','SWIROI');
    YangfigHandle = evalin('base','YangfigHandle');
    persistent myHandle
    p=[31,7,1540,5.208,20,21,36,164,0.14785,0.10];
    IData=squeeze(IBuffer);QData=squeeze(QBuffer);
    ROILA = PData(2).Region.PixelsLA(1);
    [h,~,~]=size(IData);
    point=[mod(ROILA,h),floor(ROILA/h)];
    Depth = floor(SWIROI.height/PData(2).PDelta(3));
    Width = length(PData(2).Region.PixelsLA)/Depth;
    IData1=IData(point(1)+1:point(1)+Depth,point(2)+1:point(2)+Width,1:100);
    QData1=QData(point(1)+1:point(1)+Depth,point(2)+1:point(2)+Width,1:100);
    I1=gpuArray(IData1);
    Q1=gpuArray(QData1);
    res1=SWI(Q1,I1,p);
    IData2=IData(point(1)+1:point(1)+Depth,point(2)+1:point(2)+Width,101:200);
    QData2=QData(point(1)+1:point(1)+Depth,point(2)+1:point(2)+Width,101:200);
    I2=gpuArray(IData2);
    Q2=gpuArray(QData2);
    res2=SWI(Q2,I2,p);
    res1=gather(res1);
    res2=gather(res2);
    res2(:,102:200)=res1(:,102:200);
    res2(:,99)=res2(:,99)*0.7+res1(:,99)*0.3;
    res2(:,100)=res2(:,100)*0.5+res1(:,100)*0.5;
    res2(:,101)=res2(:,101)*0.3+res1(:,101)*0.7;
    assignin('base','res',res2);
    figure(YangfigHandle), if ishandle(myHandle), delete(myHandle); end
    myHandle = imagesc(res2(:,37:164),[0,30000]);
end
...
function SWIon(varargin)
    ...
    if ~evalin('base','exist(''YangfigHandle'',''var'')')

        SWIROI = evalin('base','SWIROI');
        
        YangfigHandle = figure('Name','杨氏模量',...
            'NumberTitle','off','Visible','on',...
            'Position',[Resource.DisplayWindow(1).Position(1)+Resource.DisplayWindow(1).Position(3), ... % left edge
            Resource.DisplayWindow(1).Position(2), ... % bottom
            [SWIROI.width+3,SWIROI.height]*7], ...            % width, height
        ...
        assignin('base','YangfigHandle',YangfigHandle);
    else
        set(evalin('base','YangfigHandle'),'Visible','on');
    end
